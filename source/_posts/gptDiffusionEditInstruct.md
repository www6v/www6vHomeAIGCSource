---
title: InstructPix2Pix
date: 2024-04-16 16:25:58
tags:
  - Diffusion
categories: 
  - AIGC
  - Diffusion 
---

<p></p>
<!-- more -->


# 论文

- 论文地址
[InstructPix2Pix: Learning to Follow Image Editing Instructions](https://arxiv.org/pdf/2211.09800)
- 开源地址
[Repo](https://github.com/timothybrooks/instruct-pix2pix) git
- Project page
[Project page](https://www.timothybrooks.com/instruct-pix2pix)

# **Method**

这章就讲两件事：1）如何**生成数据集**（章节3.1）；2）如何**基于上一步生成的训练数据，训练一个图像编辑扩散模型**（章节3.2）；


{% asset_img  '94xbrqlt.bmp' %}

{% asset_img  '72gzhwfl.bmp' %}

# 参考

[InstructPix2Pix：用指令给图像做修改](https://zhuanlan.zhihu.com/p/655135961)

# 实战

1xx. [InstructPix2Pix](https://www.notion.so/5d04c1bbd0d34be4b3fb0087cb670efd?pvs=21)  diffusers
1xx. [InstructPix2Pix](https://www.bilibili.com/video/BV1Go4y1M7cK?p=3) V
[Code Repo](https://github.com/www6v/Diffusion_Training_Examples) git
